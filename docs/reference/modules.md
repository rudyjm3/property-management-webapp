...
- **Web UI**: `<ModuleGate module="...">` (`apps/web/components/ModuleGate.tsx`)
  ...
  everyone else). **No dedicated vendor management page or nav item was
  built** — there was no vendor UI at all before this change (only an
  unused `api.vendors.list()` client method), and building a full
  vendor list/detail/CRUD page was cut from this change's scope in favor of
  getting the schema, gating, and aggregation logic right; base vendor CRUD,
  work-history/rating endpoints, and preferred-vendor-assignment endpoints
  are all real and tested but currently API-only, reachable via direct API
  calls or a future UI change, not from any page in `apps/web` today.
...
multiple vendors for the same property. **No dedicated vendor management
web page was built** — see the Web UI note above for what's cut. See the
module table above and `BUILD_OUTLINE.md` §14 Module 5 for the full
breakdown of what shipped vs. what's simplified.
