...
| `/organizations/:orgId/properties` | `routes/properties.ts` (nests `/:propertyId/units` → `routes/units.ts`, which nests `/:unitId/appliances` → `routes/appliances.ts` and `/:unitId/inspections` → `routes/inspections.ts`) | `requireAuth`, `requireOrg`, plus `requireModule('unit_intelligence')` applied at the `appliances` router mount and `requireModule('inspections_compliance')` applied at the `inspections` router mount, both inside `units.ts` (rest of `properties.ts`/`units.ts` is ungated) |
