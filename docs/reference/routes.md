...
| `/organizations/:orgId/state-eviction-rules` | `routes/state-eviction-rules.ts` | `requireAuth`, `requireOrg`, `requireModule('eviction_management')` — the jurisdiction reference-data table (`GET` list/get is open to any org role; `POST`/`PATCH` upsert/correct a rule, `owner`/`manager` only). Global reference data, not org-scoped, but mounted under an org path for a consistent auth chain like every other route here |
