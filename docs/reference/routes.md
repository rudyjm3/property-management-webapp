...
| `/organizations/:orgId/evictions` | `routes/evictions.ts` | `requireAuth`, `requireOrg`, `requireModule('eviction_management')` (entirely net-new, whole router gated at its mount — same pattern as Modules 2/4/6), plus `requireRoles(['owner','manager'])` on every route in the file including `GET` (mirrors `leases.ts`'s blanket manager-only gating — eviction records carry sensitive tenant legal information). Notice CRUD + lifecycle-transition endpoints (`resolve`/`file`/`court-date`/`judgment`/`writ`/`complete`/`dismiss`), each its own `POST`, plus `GET`/`PATCH`/`DELETE` on the base notice record |
| `/organizations/:orgId/state-eviction-rules` | `routes/state-eviction-rules.ts` | `requireAuth`, `requireOrg`, `requireModule('eviction_management')` — the jurisdiction reference-data table, `GET` list/get only, open to any org role. Global reference data (no `organizationId`), not org-scoped, but mounted under an org path for a consistent auth chain like every other route here. **Deliberately no write endpoint** — `StateEvictionRule` is shared across every tenant and this codebase's RBAC has no platform-admin concept, so an owner/manager route here would let one customer overwrite the data every other customer's deadlines depend on (flagged by review on the first PR); `eviction.service.ts`'s `createOrUpdateStateEvictionRule`/`updateStateEvictionRule` exist for direct database/ops use, not wired to a route |
